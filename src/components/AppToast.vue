...
export default {
  name: 'AppToast',
  data() {
    return { visible: false, icon: '✓', text: '', sub: '', timer: null }
  },
  methods: {
    show(icon, text, sub) {
      ...
      this.$nextTick(() => {
        this.visible = true
        this.timer = setTimeout(() => { this.visible = false }, 3100)
      })
    }
  }
}
...
.toast {
  ...
  font-family: 'Geist', sans-serif; min-width: 210px; pointer-events: none;
}
...
.toast.show .t-prog { animation: drain 3s linear forwards; }
