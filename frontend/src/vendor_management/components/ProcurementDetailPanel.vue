    ...
    <template v-if="req">
      ...
      <div class="dp-head">
        ...
        <div class="dp-chips-row">
          <span class="chip" :class="priorityClass(req.priority)">{{ req.priority }} Priority</span>
      ...
      <div class="dp-scroll">
        ...
        <div>
          ...
          <div class="dp-kv"><span class="k">Part</span>       <span class="v">{{ req.partName }}</span></div>
          <div class="dp-kv"><span class="k">Size</span>       <span class="v mono">{{ req.size }}</span></div>
          <div class="dp-kv"><span class="k">Quantity</span>   <span class="v mono">{{ req.quantity ? req.quantity + ' pcs' : '—' }}</span></div>
          <div class="dp-kv"><span class="k">Requested on</span><span class="v">{{ req.date }}</span></div>
          <div class="dp-kv"><span class="k">Required by</span> <span class="v">{{ req.requiredBy || '—' }}</span></div>
        </div>
        ...
        <!-- Notes -->
        <div v-if="req.notes">
          <div class="dp-sec-label">Notes</div>
          <div class="dp-note">{{ req.notes }}</div>
        </div>
...
export default {
  ...
  methods: {
    priorityClass(p) {
      return { High: 'chip-high', Medium: 'chip-medium', Low: 'chip-low' }[p] || ''
    },
    statusClass(s) {
      return s === 'received' ? 'chip-received' : 'chip-pending'
    },
    statusLabel(s) {
...
<style scoped>
/* Slide-in panel — same pattern as OrderDetailPanel */
.dp {
...
.dp-pid { font-family: 'Geist Mono', monospace; font-size: 10.5px; color: var(--blue); font-weight: 600; }
.dp-close-btn {
...
.dp-name { font-size: 16px; font-weight: 600; color: var(--ink); letter-spacing: -0.3px; margin-bottom: 7px; }
...
.dp-lead { display: flex; align-items: center; gap: 5px; }
.dp-lead-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
...
.dp-done-badge {
  ...
  padding: 8px; background: var(--green-dim); border-radius: 6px;
}
</style>
